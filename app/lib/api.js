...
// // This simulates a real API with endpoints and proper structure
...
//     // For now, we'll simulate API responses with local data
...
//       data = await getMockActivities(endpoint, options)
//     } else {
...
//     if (params.minRelevance) queryParams.set("minRelevance", params.minRelevance)
...
//  */
// export async function getFeaturedActivities(limit = 4) {
//   try {
...
//     const categories = ["All Categories", ...new Set(activities.map((activity) => activity.category))]
...
// /**
//  * Get mock activities data with filtering
//  * @param {string} endpoint - API endpoint
//  * @param {Object} options - Request options
//  * @returns {Promise<Array>} - Filtered activities
//  */
// async function getMockActivities(endpoint = "", options = {}) {
...
//   // Check if this is a single activity request
//   if (endpoint.match(/\/activities\/[^/]+$/)) {
//     const id = endpoint.split("/").pop()
//     if (id === "featured") {
//       // Handle featured activities request
//       const limit = params.limit ? Number.parseInt(params.limit) : 4
//       return mockActivitiesData.sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, limit)
//     } else {
//       // Handle single activity request
//       const activity = mockActivitiesData.find((a) => a.id === id)
...
//     filteredActivities = filteredActivities.filter((activity) => activity.relevanceScore >= minScore)
//   }
...
//   description: "Discover the vibrant street art scene with local artists who know the stories behind the murals.",
...
//   description: "Rejuvenate your mind and body with daily yoga sessions by the ocean led by certified instructors.",
...
//   description: "Sample exquisite local wines at renowned vineyards with expert sommeliers guiding your experience.",
...
//   description: "Navigate thrilling trails with stunning mountain views. Routes available for different skill levels.",
...
//   description: "Soar above picturesque landscapes at sunrise for unforgettable views and photo opportunities.",
...
//   description: "Learn traditional recipes passed down through generations in this hands-on cooking workshop.",
...
//   relevanceScore: 6,
//   image: "/kayaking.jpg",
//   description: "Paddle through scenic waterways and discover hidden coves accessible only by water.",
...
//   description: "Chase the magical Aurora Borealis with expert guides who know the best viewing locations.",
...
//   relevanceScore: 8,
//   image: "/cameltrek.jpg",
//   description: "Experience the desert the traditional way with a camel trek to a Bedouin camp.",
//   location: "Dubai, UAE",
//   price: 85,
//   duration: "Half day",
//   groupSize: "Medium group",
//   languages: ["English", "Arabic"],
//   included: ["Camel ride", "Traditional dinner", "Entertainment", "Henna painting"],
//   reviews: {
//     average: 4.6,
//     count: 143,
//   },
 ...
 * @returns {Promise<Array>} - Featured activities array
 */
export async function getFeaturedActivities(limit = 16) {  // Changed from 4 to 16
  try {
    const activities = await fetchAPI(`/activities/featured?limit=${limit}`, {
      method: "GET",
    })
...
export async function getCategories() {
  try {
    ...
    const mockCategories = ["All Categories", ...new Set(mockActivitiesData.map((activity) => activity.category))]
